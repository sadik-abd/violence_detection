...
class I3DHead(nn.Module):
    """Classification head for I3D, refactored to use only PyTorch.
    ...
    Args:
        ...
        init_std (float): Std value for layer weight initialization. Default: 0.01.
    """
    ...
    def __init__(self,
        ...
        if self.spatial_type == 'avg':
            # Use `nn.AdaptiveAvgPool3d` to adaptively match the (T, H, W) dimensions.
    ...
    def init_weights(self):
        """Initiate the parameters from scratch using normal distribution."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): The input data of shape [N, C, T, H, W].
        ...
        x = x.view(x.size(0), -1)  # [N, in_channels]
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.cls_head = I3DHead(num_classes=2,in_channels=768,spatial_type="avg")
        self._freeze_stages()
...
if __name__ == "__main__":
    ...
    # with torch.no_grad():
    #     traced_model = torch.jit.script(model, example_inputs={"x":dummy_x})
    
    # # Save the traced model
    # traced_model.save("swin_transformer3d_traced.pt")
    # print("Traced model saved as 'swin_transformer3d_traced.pt'")
